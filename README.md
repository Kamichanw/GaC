...
- **CONFIG_API_SERVER**: List of models to be used in the ensemble. Each model configuration includes:
  ...
  - **priority**: If all models are 'supportive', every token will be ensembled at each generation step. For threshold-based ensembling, set the gate model's priority to "primary".
